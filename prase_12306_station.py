...
pool = None
ist_sql_patter = "insert into train_line_stop_tc(train_code,name,sequence,arrive_time,staytime,days,duration,depart_time,grade,train_no)value('%s','%s',%s,'%s',%s,%s,%s,'%s','%s','%s');\n"
...
ss = open('12306station_415_s.sql','w')
log = open('12306station_415_s2s.csv','w')
base_train_list_path = "all_12306stations_415/"
total = 0
...
def parse_ss_json(content,train_code):
    ...
    if  json_ss['data']:
        ...
        name_list = {}
        days = 0
        train_no = sqlSessionFactory.select(get_train_no % train_code)[0][0]
        for i,d in enumerate(data):
            ...
            if i == len(data)-1:
                depart_time = 'null'
            item_sql = ist_sql_patter % (train_code,d['station_name'],int(d['station_no']),arrive_time,stayTime,days,duration,depart_time,typeName,train_no)
            ss.write(item_sql)
